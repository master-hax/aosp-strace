...
# logs ar pulled automatically from the device to the root of the tree
# (strace-log-*)
...
strace-%-test: TEST_TMP_DIR := /data/local/tmp
strace-%-test:
	@printf >&2 "\n$*: RUNNING\n" ; \
	adb shell rm $(TEST_TMP_DIR)/strace-log-$* ; \
...
adb-sync:
	@while [ -z "$$CONTINUE" ]; do \
    read -r -p "run adb sync? [y/N] " CONTINUE; \
  done ; \
  if [ $$CONTINUE == "y" ]; then \
  	printf >&2 ">>> sync started\n" ; \
    adb sync ; \
    printf >&2 ">>> sync finished\n" ; \
  fi ; \

run-strace-tests: adb-sync $(foreach file, $(src_files), strace-$(basename $(file))-test)
